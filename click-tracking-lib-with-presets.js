	...
	function _clickTrackingLib(){
		...
		var matchPresets = [
			function( a ){
				...
				return ( a.getAttribute("rel") && a.getAttribute("rel").match(RegExpCache[1]) ) ? true : false;
			},
			function( a ){
			...
			function( a ){
				// matches file names
				return ( a.pathname.match(RegExpCache[2]) ) ? true : false;
			},
			function( a ){
			...
			function( a ){
				...
				return ( a.protocol.match(RegExpCache[3]) && a.hostname == document.location.hostname ) ? true : false;
			},
			function( a ){
		...
		this.attachTrackingFunctions = function( links, delay ){
			...
			var links = ( links.href ) ? [ links ] : ( links.length ) ? links : ( !links ) ? document.links : [];
